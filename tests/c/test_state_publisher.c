...
bool mqtt_manager_publish(const char *topic, const char *payload, int qos, bool retain)
{
    ...
    memcpy(message->topic, topic, strlen(topic) + 1);
...
static const message_t *find_message(const char *suffix)
{
    for (unsigned i = 0; i < message_count; ++i) {
        ...
        if (strlen(topic) < strlen(suffix)) continue;
...
static void read_coordinator_health(void *context,
                                    state_publisher_coordinator_health_t *out)
{
    (void)context;
    *out = (state_publisher_coordinator_health_t){0};
}
int main(void)
{
    state_publisher_config_t cfg = {.app_config = &config,
        .read_coordinator_health = read_coordinator_health};
